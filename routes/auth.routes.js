...
router.post(
  ...
  [
    ...
    check("password", "password mast be longer 3 and shorter 12").isLength({
      min: 3,
      max: 12,
    }),
  ],
  async (req, res) => {
    try {
      ...
      if (candidate) {
        return res.status(404).json({ message: "user has registration" });
      }
...
router.post(login, async (req, res) => {
  try {
    ...
    const token = jwt.sign({ id: candidate.id }, secretKey, {
      expiresIn: "1h",
    });
    return res.json({
      token,
